...
Raspberry Pi OS 中 XDP 出现期望外现象的位置是 Linux API \textt{bpf\_xdp\_adjust\_tail} 函数。
在此函数中，内核将检查当前网卡驱动支持的最大帧长，从而保证在使用 \textt{bpf\_xdp\_adjust\_tail} 函数扩展帧长时不会超过最大帧长。
这需要网卡驱动设置了对应的最大帧长，否则内核将会将最大帧长初始化为一个极大值。
而 Raspberry Pi OS 中对应的网卡驱动即没有设置此值，导致上述检查失败，\textt{bpf\_xdp\_adjust\_tail} 函数提前返回。
但由于本项目中仅使用 \textt{bpf\_xdp\_adjust\_tail} 函数收缩帧长、不会导致帧过长的问题，所以我们可以安全地将此检查移除，从而使得测试能够在 Raspberry Pi OS 上正常进行。
